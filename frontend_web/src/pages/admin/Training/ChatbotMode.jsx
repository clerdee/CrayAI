...
import { 
  Check, Clock, Search, Plus, MessageSquare, Edit3, Trash2, 
  Loader2, XCircle, Sparkles, AlertTriangle, AlertCircle, 
  ...
  return (
    <div className="animate-in fade-in zoom-in duration-300 relative">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 min-h-[600px] flex flex-col">
            ...
            <div className="p-6 border-b border-slate-100 flex flex-col xl:flex-row justify-between items-center gap-4">
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl gap-1 w-full xl:w-auto overflow-x-auto">
                    {[
                        ...
                        >
                            {tab.label}
                            <span className={`text-[10px] px-2 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-slate-100 text-slate-600' : 'bg-white text-slate-400'}`}>
                                {tab.count}
                            </span>
                        </button>
                    ))}
                    ...
                    <button 
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 whitespace-nowrap ${
                            activeTab === 'Gemini' 
                            ? 'bg-purple-100 text-purple-800 shadow-sm ring-1 ring-purple-200' 
                            : 'text-slate-500 hover:text-purple-600'
                        }`}
                    >
                        <Sparkles className="w-4 h-4" /> Gemini Answers 
                        {stats.gemini_fallback_count > 0 && (
                            <span className="bg-purple-500 text-white text-[10px] px-1.5 rounded-full shadow-sm animate-pulse">
                                {stats.gemini_fallback_count}
                            </span>
                        )}
                    </button>
                ...
                <div className="flex gap-3 w-full xl:w-auto">
                    <div className="relative flex-1 xl:w-64 group">
                        ...
                        <input type="text" placeholder="Search knowledge..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-8 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-teal-500 transition-all" />
            ...
            {activeTab === 'Unanswered' ? (
                <UnansweredTab key={lastUpdated} searchQuery={searchQuery} onConvertToQa={handlePromoteLog} />
            ) : activeTab === 'Gemini' ? (
                <GeminiTab key={lastUpdated} searchQuery={searchQuery} onConvertToQa={handlePromoteLog} />
            ) : (
                    ...
                    {filteredDataset.length > 0 && (
                        <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-b-3xl">
                            ...
                            <div className="flex items-center gap-1">
                                <button onClick={prevPage} disabled={currentPage === 1} className={`p-2 rounded-lg border border-slate-200 transition-all ${currentPage === 1 ? 'opacity-50 cursor-not-allowed bg-slate-100 text-slate-400' : 'bg-white hover:bg-teal-50 hover:border-teal-200 text-slate-600 hover:text-teal-600'}`}>
                                    <ChevronLeft className="w-4 h-4" />
                                </button>
                                
                                {pageNumbers.map(number => (
                                    <button key={number} onClick={() => paginate(number)} className={`px-3 py-1.5 text-sm font-bold rounded-lg border transition-all ${currentPage === number ? 'bg-teal-600 text-white border-teal-600 shadow-md shadow-teal-200' : 'bg-white text-slate-600 border-slate-200 hover:bg-teal-50 hover:border-teal-200 hover:text-teal-600'}`}>
                                        {number}
                                    </button>
                                ))}

                                <button onClick={nextPage} disabled={currentPage === totalPages} className={`p-2 rounded-lg border border-slate-200 transition-all ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed bg-slate-100 text-slate-400' : 'bg-white hover:bg-teal-50 hover:border-teal-200 text-slate-600 hover:text-teal-600'}`}>
